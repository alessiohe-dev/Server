...
?>
<main class="account-page"><section class="account-head"><div><p class="kicker">Verbundenes Spielerprofil</p><h1>Guten Tag, <em><?= e($profile['username'] ?? '') ?>.</em></h1><p>Deine aktuellen Werte aus dem Unity-Programm und TiDB Cloud.</p></div><div><a class="button" href="<?= e(mega_download_url()) ?>" target="_blank" rel="noopener">Programm öffnen ↗</a><form method="post" action="/logout.php"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><button class="outline-button">Abmelden</button></form></div></section><?php if ($dbError): ?><div class="notice error"><?= e($dbError) ?></div><?php endif; ?><section class="account-stats"><article><span>Spielerlevel</span><strong><?= (int)($profile['level'] ?? 1) ?></strong><small>DartSystem-Level</small></article><article><span>Experience</span><strong><?= number_format((int)($profile['experience'] ?? 0), 0, ',', '.') ?></strong><small>Gesammelte XP</small></article><article><span>Genauigkeit</span><strong><?= number_format($averageAccuracy, 1, ',', '.') ?>%</strong><small>Letzte <?= count($progress) ?> Level</small></article><article><span>Erfasste Darts</span><strong><?= number_format($totalDarts, 0, ',', '.') ?></strong><small>Synchronisiert</small></article></section><div class="account-grid"><section class="data-panel"><div class="panel-heading"><div><p class="kicker">Training</p><h2>Letzter Fortschritt</h2></div></div><?php if (!$progress): ?><div class="empty-state"><b>Noch kein Fortschritt</b><span>Spiele dein erstes Level im Programm.</span></div><?php else: ?><div class="data-list"><?php foreach ($progress as $row): ?><article><div><b><?= e($row['level_id'] ?? '') ?></b><small><?= (int)($row['darts_thrown'] ?? 0) ?> Darts · <?= (int)($row['attempts'] ?? 0) ?> Versuche</small></div><strong><?= number_format((float)($row['accuracy'] ?? 0), 1, ',', '.') ?>%</strong></article><?php endforeach; ?></div><?php endif; ?></section><section class="data-panel"><div class="panel-heading"><div><p class="kicker">Bestwerte</p><h2>Highscores</h2></div><a href="/rangliste.php">Rangliste →</a></div><?php if (!$highscores): ?><div class="empty-state"><b>Noch keine Highscores</b><span>Deine Bestwerte erscheinen hier.</span></div><?php else: ?><div class="data-list highscores"><?php foreach ($highscores as $index => $row): ?><article><span>#<?= $index + 1 ?></span><div><b><?= e($row['level_id'] ?? '') ?></b><small><?= e(date('d.m.Y', strtotime((string)($row['created_at'] ?? 'now')))) ?></small></div><strong><?= number_format((int)($row['score'] ?? 0), 0, ',', '.') ?></strong></article><?php endforeach; ?></div><?php endif; ?></section></div></main>
